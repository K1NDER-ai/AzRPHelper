...
local status_bar = imgui.ImBool(true)
...
monitoring_func =
{
	[0] = '[FastConnect]', '[AntiAfk]', '[BufferCleaner]', '[TuningFix]',
}
...
function imgui.OnDrawFrame()
	...
	if status_bar.v then
		renderDrawBoxWithBorder(-2, resY - 30, resX + 3, 30, 0xBF191919, 2, 0xFF000000) 
		renderFontDrawText(font, string.format(''..monitoring_func[0]..' '..monitoring_func[1]..' '..monitoring_func[2]..' '..monitoring_func[3]..''), 5, (resY - 30) + 7, 0xFFFFFFFF)
	end
	...
	if main_window_state.v then
		...
		if menu == 3 then
			...
			imgui.Text(u8"\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tВерсия: 1.3.b(Beta)")
...
function main()
	...
	while true do
		...
		wait(0)
		if wasKeyPressed(load_settings.settings_script.activate_script) and not sampIsChatInputActive() and not sampIsDialogActive() and not isPauseMenuActive() and not isSampfuncsConsoleActive() then
			main_window_state.v = not main_window_state.v
		end
		imgui.Process = main_window_state.v
		imgui.Process = status_bar.v
	end
end
