...
function imgui.TextColoredRGB(text, id)
	...
	if openChat then
		...
		if imgui.BeginPopupContextItem() then
			if imgui.Button(u8'Редактировать', imgui.ImVec2(-1, 0)) then editChatLineId = id imgui.StrCopy(editChatLineColor, messages[id].color:match('{(.+)}')) imgui.StrCopy(editChatLine, messages[id].text) imgui.StrCopy(editChatLineTime, messages[id].timestamp:match('%[(.+)%]')) imgui.OpenPopup('EditChatLine1') end
			if imgui.Button(u8'Скопировать в буфер обмена') then setClipboardText(u8:decode(messages[id].text)) imgui.CloseCurrentPopup() end
			if imgui.Button(u8'Скопировать в поле чата', imgui.ImVec2(-1, 0)) then imgui.StrCopy(inputChat, messages[id].text) imgui.CloseCurrentPopup() end
			if imgui.Button(u8'Удалить', imgui.ImVec2(-1, 0)) then table.remove(messages, id) setup_current_scroll = setup_current_scroll - imgui.GetTextLineHeightWithSpacing() imgui.CloseCurrentPopup() end
